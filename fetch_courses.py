...
鉴权：
    登录 1.tongji.edu.cn 后，浏览器 F12 -> Console 执行
        sessionStorage.getItem('sessionid')
    把返回值填到下面的 X_TOKEN（这是接口的 X-Token，不是 Cookie）。
"""
...
X_TOKEN = "xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx"   # 必填：见文件头说明
COOKIE = ""                            # 可选：整段 Cookie，一般留空即可
...
_API_COND = {
    ...
    "form_learning": "formLearning",
}
_DICT_KEYS = ["X_KCXZ", "X_XQ", "X_PYCC", "K_XXXS"]
_DICT_LABEL = {"X_KCXZ": "课程性质(nature)", "X_XQ": "校区(campus)",
               "X_PYCC": "培养层次(training_level)", "K_XXXS": "学习形式(form_learning)"}
...
def cmd_options():
    ...
    print("== 当前学期 ==")
    print("  %s  %s\n" % (cur_id, cur_name))
    print("== 全部学期（FILTERS['calendar'] 可填 id 或 'current'）==")
    ...
    for dk, label in _DICT_LABEL.items():
        print("  -- %s --" % label)
        for code, name in (list_dicts().get(dk) or {}).items():
            print("    %s  %s" % (code, name))
...
def write_outputs(rows, cal_id, cond):
    ...
    with open(base + ".csv", "w", encoding="utf-8-sig", newline="") as f:
        ...
        w.writerow(keys)
        for r in rows:
            w.writerow([r.get(k) if not isinstance(r.get(k), (dict, list)) else json.dumps(r.get(k), ensure_ascii=False) for k in keys])
    summary = "; ".join("%s=%s" % (k, v) for k, v in cond.items() if v)
    print("完成：%d 条  筛选[%s]" % (len(rows), summary))
    print("输出: %s-raw.json / %s.csv" % (base, base))
...
def main():
    if len(sys.argv) > 1 and sys.argv[1] == "options":
        cmd_options()
        return
    if X_TOKEN.startswith("在此粘贴"):
        print("请先填写 X_TOKEN（浏览器 Console 执行 sessionStorage.getItem('sessionid')）")
        sys.exit(1)
